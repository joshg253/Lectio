...
- **Email Article rules** — server-side automation for Email Article rules fires at every feed refresh. Two delivery modes: *Immediately* sends one email per matching new article (capped at 10 per refresh cycle); *Batch* queues articles and flushes as a digest when a count threshold is hit or during the daily maintenance window. The *Cc me* option adds your profile email as Cc, suppressed when it already matches the To address.
