...
- **Feed Properties / Image Troubleshooter** — right-click a feed → Properties to inspect feed metadata, health, and post counts. The Image Display section controls three per-feed flags:
  ...
  - *Caption source* — two checkboxes (**Title** / **Alt**) select which HTML attribute from the lead image is used as the caption text. Both unchecked saves "none" (no caption); **↺ Auto** resets to the default title-preferred behavior. The selected text is pre-loaded on first open so it never pops in on second view.
  - *Post thumbnail* — choose what appears in the post list: per-entry auto image, feed favicon, a pinned custom URL, or one of the strategy-detected images. Setting a fixed thumbnail (favicon or custom URL) locks the post list thumbnail and prevents it from being replaced when opening an entry.
  - *Article image source* — lock the feed to a specific image extraction strategy: **Auto-detect**, **Webcomic** (source-page scrape for comic strips), **Artwork** (hoists the first image from feed content to the top, suited for art-portfolio feeds like ArtStation), **Feed content only**, **Source scraping**, **Media RSS**, or **None**. ArtStation feeds are auto-assigned **Artwork** strategy; feeds in folders whose name contains "comic" are auto-assigned **Webcomic**.
...
- **GUID-churn suppression** — after each feed refresh, entries that reappear with a new GUID but the same URL slug (caused by CMS migrations or permalink rebuilds) are automatically marked read.
