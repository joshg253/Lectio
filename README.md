...
- **Podcast audio-feed suggestion** — some sites publish a *website* feed (rich show notes, no audio) while the MP3s live in a separate feed on a podcast host (Libsyn, Buzzsprout, Transistor, Megaphone, Simplecast, …). When an entry has no playable audio, Lectio scans the episode page (in the background) for that podcast-host feed and, if found and not already subscribed, shows a one-click **Subscribe to the audio feed** banner on the article — so you can keep the notes feed and add the audio feed
