...
- **Per-folder refresh cadence** — set a custom polling interval (1 min to once a day) that overrides the global interval for feeds in that folder; change it inline from the refresh dropdown on each row in Settings → Feeds → Folders, or via right-click a folder → Properties
