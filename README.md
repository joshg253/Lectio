...
- **Inline podcast player** — entries with a podcast audio enclosure get a native `<audio>` player (plus a download link) at the top of the article. Audio is detected by MIME type or by the file extension on the URL path, so query-string/auth-tokened URLs and untyped enclosures still match; if a feed has no enclosure but its entry link points straight at an audio file, that's used instead. Audio that lives only in `<media:content>` / `<media:group>` (which the underlying feed library drops) is recovered too: the raw feed is re-parsed in the background on first open and the resulting per-entry audio URL is cached, so the player fills in without slowing the article open
- **Podcast audio from a separate host feed** — some sites publish a *website* feed (rich show notes, no audio) while the MP3s live in a separate feed on a podcast host (Libsyn, Buzzsprout, Transistor, Megaphone, Simplecast, …). When an entry has no playable audio, Lectio finds that host feed (referenced on the episode page) in the background and **borrows the audio into the website feed**: it matches each post to the right episode (by title, then episode number) in the host feed and plays its MP3 inline — so you get notes *and* audio in one subscription, no duplicate feed. If an episode can't be matched, the article instead shows a one-click **Subscribe to the audio feed** banner (only when that feed isn't already subscribed). These discovery/borrow fetches identify honestly as Lectio by default, escalating to a browser User-Agent only when a host actually refuses (HTTP 403) — the Cloudflare-gated podcast hosts — so we never spoof a site that's happy to serve us
- **Attachments** — non-audio enclosures (magazine PDFs, EPUBs, and other downloadable files) are listed in an **Attachments** section at the foot of the article, each as a direct download link with its file name and size (placeholder sizes are hidden). Audio enclosures are skipped here since they already appear as the inline player above
- **Embeds that actually render** — `reader`/feedparser sanitize feeds by *destroying* anything off their allowlist (iframes, SVG, MathML, audio/video), so embedded videos and players silently vanish. Lectio parses feeds with that sanitization disabled and applies its **own** allowlist instead, keeping `<iframe>` embeds from a curated trusted-host list (YouTube, Vimeo, Dailymotion, Twitch, SoundCloud, Bandcamp, Spotify, Twitter/X, CodePen, Reddit, Archive.org) as sandboxed players, plus sanitized inline SVG and MathML — while still dropping scripts, event handlers, and unsafe URLs. So a video embedded mid-article just plays
- **Recovered YouTube embeds** (legacy/backfill) — the feed library strips the embed `<iframe>` from post content (e.g. a WordPress YouTube block), leaving an empty figure with the video id gone. Lectio re-parses the raw feed (with sanitization off, where the original iframe survives), caches the recovered video id(s) per entry, and rebuilds a privacy-enhanced `youtube-nocookie` player in place — so videos embedded mid-article actually play. Recovery shares the same raw-feed re-parse as the podcast-audio fallback; a newly-seen post queues the scan and fills in on the next open
