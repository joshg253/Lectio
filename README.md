...
- **Manual tags** — tag any article from the article pane (the `sell` button); tags appear in the left-pane Tags list. Clicking a tag (sidebar list or article chip) shows **all** articles with that tag (read included); picking a folder/feed afterward restores the read filter you had before (e.g. Unread), the same way Starred/History do. Remove a single tag from a post with the `×` on its chip, or right-click any tag → **Delete tag everywhere** to strip it from every post (the tag drops out of the sidebar once nothing references it)
