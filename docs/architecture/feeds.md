...
**A naive comma-split on `srcset` shredded any CDN URL that embeds its own comma-separated transform params.** Substack's image-fetch URLs look like `.../fetch/$s_!x!,w_424,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2F...jpeg 424w` — the whole point being the transform args live in the *path*, comma-separated, ahead of the real (percent-encoded) source URL. `resolve_relative_urls`'s srcset handling used to do `raw.split(",")` to find each `"url descriptor"` candidate, which cut a URL like that off at its first internal comma; the leftover fragment (`w_424`) then got `urljoin`'d against the entry's own page as if it were a real relative path, producing a garbage same-host URL (`https://kriscox.substack.com/p/w_424`) that 404s. Reported live 2026-09-18 (kriscox.substack.com, two separate entries) as "lead img broken" — server-side everything checked out (the archived asset existed, decoded as a valid webp, and `/starred-asset/<hash>` served it correctly) because the investigation kept checking `src`, which the rewriter (`StarredArchiveService.rewrite_html_assets`) correctly points at the local copy. The browser never uses it: a `srcset` candidate that matches the viewport wins over `src` by the HTML spec, and every candidate in the shredded `srcset` was garbage.

This is the *same* bug class already found and fixed once, elsewhere: `LeadImageService._parse_srcset_urls_descending` (image-candidate discovery) got the correct WHATWG "parse a srcset attribute" scan on 2026-08-12 for this exact CDN-URL shape, but that fix never reached this second, independent srcset handler. Fixed here by porting the same scanning algorithm (`_split_srcset_candidates`, a private helper local to `html_sanitize.py`, not shared code — the two call sites want different return shapes) rather than re-deriving it: a candidate is a whitespace-delimited URL token (a `srcset` URL never contains whitespace) followed by a comma-terminated descriptor, so internal commas inside the URL itself are never mistaken for a candidate boundary. Existing entries whose stored `content_html` was written before this fix keep the shredded `srcset`; re-fetching the entry's content repairs one, same as the relative-URL case above.

**`html_sanitize.sanitize_html` parses with `lxml`, not the stdlib `html.parser`.** Profiling
against real entry bodies found ~26% of its time went to one avoidable cost (an ancestor-chain
walk on every tag, now precheck-guarded) and the rest mostly in BeautifulSoup's own tree-walking
machinery, largely independent of which parser built the tree — so the backend itself was only
ever a modest lever (~1.2-1.4x). Switched anyway since the two backends' few real divergences
(whitespace normalization, `lxml` correctly auto-closing a `<p>` before a block-level child) are
things a real browser's own HTML parsing already normalizes at render time — free upside, no
behavior change a reader would ever see.
