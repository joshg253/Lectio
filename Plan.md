...
- ~~**⚠ Failure badge**~~ *(done)*
- **Feed type presets + full persistent failure alerting UI** *(do together — both touch Feed Properties)* — split the "Article image source" dropdown into a **Feed type preset** picker (Webcomic, Artwork, Standard, etc. — sets multiple defaults at once) and a separate **Mode override** (Feed content / Source page / Media RSS). In the same pass: add Pause updates toggle, Change URL field, and full error detail display to Feed Properties. GitHub app-release preset: no thumbnail, use the changelog image already in the entry.
