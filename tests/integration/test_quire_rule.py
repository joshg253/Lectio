...
def test_rule_persists(env):
    with main.get_meta_connection() as conn:
        main.add_highlight_keyword(conn, "feed", FEED, "metal", "yellow", rule_type="quire", enabled=1)
...
def test_after_refresh_adds_matching_entries(env, monkeypatch):
    tasks = []
    monkeypatch.setattr(main.quire_service, "create_task", lambda tok, oid, name, desc="": tasks.append((oid, name)))
    with main.get_meta_connection() as conn:
        main.add_highlight_keyword(conn, "feed", FEED, "metal", "yellow", rule_type="quire", enabled=1, search_in="title")
...
def test_blank_keyword_adds_all_in_scope(env, monkeypatch):
    tasks = []
    monkeypatch.setattr(main.quire_service, "create_task", lambda tok, oid, name, desc="": tasks.append(name))
    with main.get_meta_connection() as conn:
        main.add_highlight_keyword(conn, "feed", FEED, "", "yellow", rule_type="quire", enabled=1)
    main._run_quire_rules_after_refresh({FEED})
