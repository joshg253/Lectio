...
def test_kept_and_available_for_a_live_entry(configured):
    content_html = (
        '<p>Body text.</p><a href="https://cdn.test/kept.pdf">already kept</a><a href="https://cdn.test/not-kept.epub">not kept yet</a>'
    )
    ...
    with main.archive_conn() as conn:
        ...
        conn.execute(
            "INSERT INTO archived_asset_link (feed_url, entry_id, source_url, asset_hash) VALUES (?, ?, 'https://cdn.test/kept.pdf', 'h1')",
            (FEED, ENTRY),
        )
