...
def env(tmp_path):
    ...
    for feed, eid, title in [
        ...
        reader.add_entry({"feed_url": feed, "id": eid, "title": title, "link": f"{feed}/{eid}", "summary": "x", "published": when})
    try:
        yield
    finally:
        main.close_thread_db_pools()
...
def test_rule_percent_is_clamped_on_save(env):
    with main.get_meta_connection() as conn:
        main.add_highlight_keyword(conn, "global", "", "fuzzy", "yellow", rule_type="deduplicate", enabled=1, dedup_fuzzy_pct=5)
        rule = [r for r in main.get_highlight_keywords(conn) if r["type"] == "deduplicate"][0]
...
def test_default_percent_for_a_rule_saved_without_one(env):
    with main.get_meta_connection() as conn:
        main.add_highlight_keyword(conn, "global", "", "title", "yellow", rule_type="deduplicate", enabled=1)
        rule = [r for r in main.get_highlight_keywords(conn) if r["type"] == "deduplicate"][0]
...
def test_min_words_round_trips_and_clamps(env):
    with main.get_meta_connection() as conn:
        main.add_highlight_keyword(conn, "global", "", "fuzzy", "yellow", rule_type="deduplicate", enabled=1, dedup_min_title_words=6)
        assert [r for r in main.get_highlight_keywords(conn)][0]["dedup_min_title_words"] == 6
        main.add_highlight_keyword(conn, "global", "", "fuzzy", "yellow", rule_type="deduplicate", enabled=1, dedup_min_title_words=99)
...
def test_four_word_rules_are_bumped_once_but_a_deliberate_four_survives(env):
    ...
    with main.get_meta_connection() as conn:
        main.add_highlight_keyword(conn, "global", "", "fuzzy", "yellow", rule_type="deduplicate", enabled=1, dedup_min_title_words=4)
