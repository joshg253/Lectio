...
def _seed(tmp_path):
    ...
    conn.execute("CREATE TABLE entries (id TEXT, feed TEXT, title TEXT, link TEXT, author TEXT, summary TEXT, content TEXT, read INTEGER)")
    ...
    conn.executemany(
        "INSERT INTO entries (id, feed, title, link, author, summary, content, read) VALUES (?,?,?,?,?,?,NULL,0)",
...
def test_article_body_is_searchable(tenant):
    ...
    conn = sqlite3.connect(str(tenancy.reader_db_path()))
    conn.execute(
        "INSERT INTO entries (id, feed, title, link, author, summary, content, read) VALUES (?,?,?,?,?,?,?,0)",
        ("e-body", FEED, "Untitled", "https://blog.test/h", "", "", '[{"value": "<p>a long digression about espresso machines</p>"}]'),
    )
    conn.commit()
    conn.close()
...
def test_chunks_past_the_sqlite_variable_limit(tenant):
    ...
    _seed(tenant)
    conn = sqlite3.connect(str(tenancy.reader_db_path()))
    extra = [(f"bulk-{i}", FEED, "Coffee bulk", "https://blog.test/x", "", "") for i in range(1200)]
    conn.executemany("INSERT INTO entries (id, feed, title, link, author, summary, content, read) VALUES (?,?,?,?,?,?,NULL,0)", extra)
    conn.commit()
    conn.close()
