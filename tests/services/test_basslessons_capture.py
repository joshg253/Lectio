...
IFRAME = (
    '<iframe width="740" height="416" src="https://www.youtube-nocookie.com/embed/fxoeU3vzdEw" frameborder="0" allowfullscreen></iframe>'
)
...
def test_sanitizer_keeps_the_embed_host(page, monkeypatch):
    ...
    monkeypatch.setattr(plugins, "extra_embed_html", lambda source_url, raw_html, *a, **kw: IFRAME)
    article = main._append_site_embeds("<p>body</p>", TRANS_URL, page)
...
def test_disallowed_embed_host_is_dropped(page, monkeypatch):
    ...
    monkeypatch.setattr(plugins, "extra_embed_html", lambda source_url, raw_html, *a, **kw: evil)
...
def test_no_embed_leaves_the_body_untouched(page, monkeypatch):
    monkeypatch.setattr(plugins, "extra_embed_html", lambda source_url, raw_html, *a, **kw: None)
    assert main._append_site_embeds("<p>body</p>", TRANS_URL, page) == "<p>body</p>"
...
def test_capture_is_the_article_and_nothing_else(page):
    ...
    _title, article = main.extract_full_page_article(page, TRANS_URL)
    for gone in ("Log in", "Nederlandstalige Versie", "Click here", "Leave a comment", "Searching far and wide"):
