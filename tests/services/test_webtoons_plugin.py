...
ENTRY = "https://www.webtoons.com/en/canvas/sarahs-scribbles/i-should-be-vs-i-am/viewer?title_no=677113&episode_no=1"
# The episode's own panel, straight out of the feed's <description>.
PANEL = "https://swebtoon-phinf.pstatic.net/20210813_280/1628808043747imSaI_JPEG/40f5276f-e150-4921-a052-9d1da6a6653a.jpg"
# The og:image — identical for every episode of the series.
SERIES_THUMB = "https://swebtoon-phinf.pstatic.net/20210813_258/1628807985198lpcHt_JPEG/thumbnail.jpg"
SUMMARY = f'<a href="{ENTRY}">\n<img src="{PANEL}"/>\n</a><br/>'


def test_the_episode_panel_comes_from_the_feed_body():
    assert WebtoonsPlugin().fallback_lead_image_url(entry_link=ENTRY, content_html=None, summary=SUMMARY) == PANEL


def test_content_html_is_preferred_over_summary():
    other = PANEL.replace("40f5276f", "aaaaaaaa")
    assert (
        WebtoonsPlugin().fallback_lead_image_url(
            entry_link=ENTRY,
...
def test_a_feed_body_holding_only_the_series_thumbnail_yields_nothing():
    ...
    assert WebtoonsPlugin().fallback_lead_image_url(entry_link=ENTRY, content_html=None, summary=f'<img src="{SERIES_THUMB}"/>') is None


def test_other_hosts_are_untouched():
    p = WebtoonsPlugin()
    assert p.fallback_lead_image_url(entry_link="https://example.com/post", content_html=None, summary=SUMMARY) is None
    assert p.should_bypass_cached_url(entry_link="https://example.com/post", cached_url=SERIES_THUMB) is False
...
def test_thumbnail_detection_is_basename_only():
    ...
    assert p.should_bypass_cached_url(entry_link=ENTRY, cached_url="https://swebtoon-phinf.pstatic.net/x/thumbnail.png") is True
...
class TestTapasPlugin:
    ...
    def test_the_panel_comes_from_the_feed_body(self):
        assert self._p().fallback_lead_image_url(entry_link=self.ENTRY, content_html=self.BODY, summary=None) == self.PANEL
    ...
    def test_series_and_www_hosts_are_covered_but_lookalikes_are_not(self):
        ...
        assert p.fallback_lead_image_url(entry_link="https://example.com/x", content_html=self.BODY, summary=None) is None
