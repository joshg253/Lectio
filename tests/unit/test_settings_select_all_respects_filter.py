...
def test_a_header_checkbox_selects_every_match_across_folders():
    ...
    assert "selectableFeedRows(null)" in m.group(1), "it must reuse the same filter-aware row set as the per-folder boxes"


def test_the_header_checkbox_lives_in_the_table_header():
    header = (Path(__file__).resolve().parents[2] / "templates" / "_settings_feeds_folders.html").read_text(encoding="utf-8")
    thead = re.search(r"<thead>.*?</thead>", header, re.S)
    assert thead, "the folders table should still have a header row"
    assert 'id="sfc-check-all-global"' in thead.group(0), "it belongs in the header's checkbox column, not floating above the table"
