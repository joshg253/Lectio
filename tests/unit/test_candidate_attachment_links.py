...
def test_finds_a_pdf_link():
    ...
    assert main.candidate_attachment_links_in_html(html, "https://example.test/post") == ["https://cdn.test/manual.pdf"]


def test_resolves_relative_links_against_base_url():
    html = '<a href="/files/tab.gp5">tab</a>'
    assert main.candidate_attachment_links_in_html(html, "https://example.test/post") == ["https://example.test/files/tab.gp5"]
...
def test_finds_obfuscated_base64_links_too():
    ...
    assert main.candidate_attachment_links_in_html(html, "https://example.test/post") == ["https://cdn.test/hidden.epub"]


def test_dedupes_repeated_links():
    html = '<a href="https://cdn.test/a.pdf">one</a><a href="https://cdn.test/a.pdf">two</a>'
    assert main.candidate_attachment_links_in_html(html, "https://example.test/post") == ["https://cdn.test/a.pdf"]
