...
- **Tag-filter rule.** The `tag_filter` rule type in the rules engine (`highlight_keywords`) consumes this table to tame firehose feeds: `keyword` holds the include-tags list, the `tags_exclude` column the exclude list (comma/space separated; at least one required). `_run_tag_filter` runs in `_run_automation_after_refresh` per refreshed feed in scope (and via dry-run/run-now), comparing normalized-to-normalized, and auto-marks matching unread entries read (same suppression as `mark_as_read`, logged to automation history). Semantics: exclude wins over include; **untagged entries are always kept** — a feed that stops tagging must not have its whole firehose suppressed. It runs *after* `update_feed` (the tag sink fires during parse, so the table is populated by then).
