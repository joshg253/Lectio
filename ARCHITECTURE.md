...
**Reviewing an ambiguous host by hand.** A host with more than one on-host feed (Medium, Ars Technica) is `ambiguous` — the filer can't pick a destination, so those saves stall in the worklist. Each row carries a magnifying-glass link that closes the dialog and opens the Saved view searched for that host (it reuses the sidebar's "All Saved" URL and appends `q=<host>&read_filter=all`, exactly as if the host were typed into the Saved search — the list search already matches an entry's link, so the host string filters to its saved articles). From there each is filed by hand with the per-post **Move to feed** action. Frontend-only; no new endpoint.
