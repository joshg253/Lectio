...
**A webcomic feed's own `<enclosure>` is usually not the comic either.** `extract_entry_thumbnail_url` checked `media_thumbnail` / `media_content` / `enclosures` / `links` immediately after the cache, before any source-page lookup — so on mahonoir.com the publisher's per-entry social card (`0312csss.jpg`, 1200×630) was returned and then *cached as the lead image*, and the panel extractor was never called at all. The container fix above could not surface while that was happening. `fetch_and_store_lead_images_for_feed` had always skipped the feed-XML thumbnail for webcomics; this path simply never got the same rule. It does now, but only when `include_source_lookup` is set and `fast_only` is not: in list rendering there is nothing better to offer, and a social card beats a blank.
